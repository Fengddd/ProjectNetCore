...
$.fn.extend({
    ...
    JQGrid: function (option) {
        ...
        option.pageList = option.pageList || [5, 10, 15];
    ...
    EmpSelector: function (option) {
        ...
        option.ValueType = option.ValueType || 'userId';
        option.TextType = option.TextType || 'userName',
        option.unitType = option.unitType || "";
        option.unitName = option.unitName || "";
        option.GetUsersByIdStr = option.GetUsersByIdStr || '/Easyui/WhereUserList';
        option.GetEmployee = option.GetEmployee || '/Easyui/UserList';
        var suffix = "_" + src.attr("id");
                            ...
                            "<div class='contSearch'>" +
                                "<form class='form-inline' role='form'>" +
                                    ...
                                    "<div class='form-group'>" +
                                        ...
                                        "<input type='text' class='form-control' id='UserName" + suffix + "'style='width:200px;' />" +
                                    "</div>&nbsp&nbsp" +
                                    "<div class='form-group'>" +
                                        "<input type='button' style='width:60px;' class='btn btn-info btn-sm' value='" + lang.Search + "' id='btnSearch" + suffix + "' onclick=\"$(\'#EmpGrid" + suffix + "\').datagrid(\'load\')\" />&nbsp&nbsp" +
								        "<input type='button' style='width:60px;' class='btn btn-info btn-sm' value='" + lang.Reset + "' onclick=\"$('#UnitType" + suffix + "').val('');$('#UnitName" + suffix + "').val('');$('#UserName" + suffix + "').val('');$('#EmpUl" + suffix + "').empty();$(\'#EmpGrid" + suffix + "\').datagrid(\'unselectAll\').datagrid(\'load\')\" />" +
                                    "</div>" +
                                "</form>" +
                            "</div>" +
        ...
        $EmpDialog.dialog({
            ...
            onOpen: function () {
                ...
                $EmpGrid.JQGrid({
                    ...
                    idField: 'userId', singleSelect: option.single, fitColumns: true,
                    queryParams: {
                    ...
                    columns: [[
                        ...
                        //{ field: 'userName', title: lang.UnitName, width: 20 },
                        //{ field: 'userMobile', title: lang.EmpName, width: 4, align: 'center' },
                        //{ field: 'UserMobile', title: lang.RoleName, width: 10, align: 'center' },
                        ...
                        { field: 'userName', title: '用户名', width: 7, align: 'center' },
                        { field: 'userMobile', title: '用户电话', width: 8, align: 'center' },
                        { field: 'userMail', title: '用户邮箱', width: 8, align: 'center'},
                        { field: 'userAddress', title: '用户地址', width: 8 }
                    ]],
                    onSelect: function (rowIndex, rowData) {
                        ...
                        var have = $("#EmpUl" + suffix + " li[userid=" + rowData.userId + "]");                      
                        if (have.length === 0) {
                    ...
                    onUnselect: function (rowIndex, rowData) {
                        ...
                        $("#EmpUl" + suffix + " li[userid=" + rowData.userId + "]").remove();
                    },
                    ...
                    onLoadSuccess: function (data) {
                        ...
                        for (var i = 0; i < rows.length; i++) {                         
                            ...
                            if ($("#EmpUl" + suffix + " li[userid=" + rowData.userId + "]").length == 0) {
                             
                                $EmpGrid.datagrid('unselectRow', i);
                            }
                            else
                            {
                                $EmpGrid.datagrid('selectRow', i);
                            }
                        }
